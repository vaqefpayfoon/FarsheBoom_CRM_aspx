...
{
    public partial class Porz : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, porz_title FROM dbo.inv_porz";
        ...
        {
            int srl = max_srl();
            string porz_title = ((TextBox)GridView1.FooterRow.FindControl("txt_porz_title_add")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into inv_porz(srl,porz_title) " +
            "values(@srl,@porz_title);" +
             "SELECT srl, porz_title FROM dbo.inv_porz";
            cmd.Parameters.Add("@srl", SqlDbType.TinyInt).Value = srl;
            cmd.Parameters.Add("@porz_title", SqlDbType.VarChar, 50).Value = porz_title;
            GridView1.DataSource = GetData(cmd);
            GridView1.DataBind();
        }
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from  inv_porz where " +
                "srl=@srl;" +
                "SELECT srl, porz_title FROM dbo.inv_porz";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string porz_title = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_porz_title")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update inv_porz set porz_title=@porz_title " +
             "where srl=@srl;" +
             "Select srl,porz_title From dbo.inv_porz";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.inv_porz";
