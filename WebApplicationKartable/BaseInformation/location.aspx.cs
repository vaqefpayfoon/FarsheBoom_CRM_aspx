...
namespace WebApplicationKartable
{
    public partial class location : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, location FROM dbo.bas_locate";
        ...
        {
            string location = ((TextBox)GridView1.FooterRow.FindControl("txt_location_add")).Text;
            int srl = max_srl();
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into bas_locate(srl,location) " +
            "values(@srl,@location);" +
             "SELECT srl, location FROM dbo.bas_locate";
        ...
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from bas_locate where " +
                "srl=@srl;" +
                "SELECT srl, location FROM dbo.bas_locate";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string location = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_location")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update bas_locate set location=@location " +
             "where srl=@srl;" +
             "Select srl,location From dbo.bas_locate";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.bas_locate";
