...
namespace WebApplicationKartable
{
    public partial class Carpet : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, carpet_title FROM dbo.inv_carpet";
        ...
        {
            int srl = max_srl();
            string carpet_title = ((TextBox)GridView1.FooterRow.FindControl("txt_carpet_title_add")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into inv_carpet(srl,carpet_title) " +
            "values(@srl,@carpet_title);" +
             "SELECT srl, carpet_title FROM dbo.inv_carpet";
            cmd.Parameters.Add("@srl", SqlDbType.TinyInt).Value = srl;
            cmd.Parameters.Add("@carpet_title", SqlDbType.VarChar, 50).Value = carpet_title;
            GridView1.DataSource = GetData(cmd);
            GridView1.DataBind();
            lblError.Text = "";
        }
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from  inv_carpet where " +
                "srl=@srl;" +
                "SELECT srl, carpet_title FROM dbo.inv_carpet";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string carpet_title = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_carpet_title")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update inv_carpet set carpet_title=@carpet_title " +
             "where srl=@srl;" +
             "Select srl,carpet_title From dbo.inv_carpet";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.inv_carpet";
