...
namespace WebApplicationKartable
{
    public partial class plan : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, plan_title FROM dbo.inv_plan";
        ...
        {
            string plan_title = ((TextBox)GridView1.FooterRow.FindControl("txt_plan_title_add")).Text;
            int srl = max_srl();
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into inv_plan(srl,plan_title) " +
            "values(@srl,@plan_title);" +
             "SELECT srl, plan_title FROM dbo.inv_plan";
            cmd.Parameters.Add("@srl", SqlDbType.Int).Value = srl;
            cmd.Parameters.Add("@plan_title", SqlDbType.VarChar, 30).Value = plan_title;
            GridView1.DataSource = GetData(cmd);
            GridView1.DataBind();
        }
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from inv_plan where " +
                "srl=@srl;" +
                "SELECT srl, plan_title FROM dbo.inv_plan";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string plan_title = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_plan_title")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update inv_plan set plan_title=@plan_title " +
             "where srl=@srl;" +
             "Select srl,plan_title From dbo.inv_plan";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.inv_plan";
