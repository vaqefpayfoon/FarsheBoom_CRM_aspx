...
namespace WebApplicationKartable
{
    public partial class Cities : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, city_name FROM dbo.bas_city";
        ...
        {
            int srl = max_srl();
            string city_name = ((TextBox)GridView1.FooterRow.FindControl("txt_city_name_add")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into bas_city(srl,city_name) " +
            "values(@srl,@city_name);" +
             "SELECT srl, city_name FROM dbo.bas_city";
        ...
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from bas_city where " +
                "srl=@srl;" +
                "SELECT srl, city_name FROM dbo.bas_city";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string city_name = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_city_name")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update bas_city set city_name=@city_name " +
             "where srl=@srl;" +
             "Select srl,city_name From dbo.bas_city";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.bas_city";
