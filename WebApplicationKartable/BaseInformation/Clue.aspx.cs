...
{
    public partial class Clue : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, meet_title FROM dbo.bas_sale_clue";
        ...
        {
            string meet_title = ((TextBox)GridView1.FooterRow.FindControl("txt_meet_title_add")).Text;
            int srl = max_srl();
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into bas_sale_clue(srl,meet_title) " +
            "values(@srl,@meet_title);" +
             "SELECT srl, meet_title FROM dbo.bas_sale_clue";
            cmd.Parameters.Add("@srl", SqlDbType.Int).Value = srl;
            cmd.Parameters.Add("@meet_title", SqlDbType.VarChar, 50).Value = meet_title;
            GridView1.DataSource = GetData(cmd);
            GridView1.DataBind();
            lblError.Text = "";
        }
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from bas_sale_clue where " +
                "srl=@srl;" +
                "SELECT srl, meet_title FROM dbo.bas_sale_clue";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string meet_title = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_meet_title")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update bas_sale_clue set meet_title=@meet_title " +
             "where srl=@srl;" +
             "Select srl,meet_title From dbo.bas_sale_clue";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.bas_sale_clue";
