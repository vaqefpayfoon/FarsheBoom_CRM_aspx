...
namespace WebApplicationKartable
{
    public partial class Brands : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "SELECT srl, brand_name FROM dbo.inv_brand";
        ...
        {
            int srl = max_srl();
            string brand_name = ((TextBox)GridView1.FooterRow.FindControl("txt_brand_name_add")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into inv_brand(srl,brand_name) " +
            "values(@srl,@brand_name);" +
             "SELECT srl, brand_name FROM dbo.inv_brand";
            cmd.Parameters.Add("@srl", SqlDbType.Int).Value = srl;
            cmd.Parameters.Add("@brand_name", SqlDbType.VarChar, 50).Value = brand_name;
            GridView1.DataSource = GetData(cmd);
            GridView1.DataBind();
        }
        protected void DeleteCustomer(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "delete from  inv_brand where " +
                "srl=@srl;" +
                "SELECT srl, brand_name FROM dbo.inv_brand";
        ...
        protected void UpdateCustomer(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string brand_name = ((TextBox)GridView1.Rows[e.RowIndex].FindControl("txt_brand_name")).Text;
            SqlConnection con = new SqlConnection(strConnString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update inv_brand set brand_name=@brand_name " +
             "where srl=@srl;" +
             "Select srl,brand_name From dbo.inv_brand";
        ...
        private int max_srl()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT ISNULL((MAX(srl) + 1),1) FROM dbo.inv_brand";
